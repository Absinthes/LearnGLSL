...
void main() {
  ...
  vec2 p = vUv;
  float ty = time * speed * 17.346f;
  float yt = p.y - ty;

	//thick distortion
  float offset = noise2d(vec2(yt * 3.0f, 0.0f)) * 0.2f;
  offset = offset * thickDistort * offset * thickDistort * offset;
	//fine distortion
  offset += noise2d(vec2(yt * 50.0f, 0.0f)) * fineDistort * 0.002f;

	//combine distortion on X with roll on Y
  gl_FragColor = texture2D(uTexture, vec2(fract(p.x + offset), fract(p.y - time * rollSpeed)));
